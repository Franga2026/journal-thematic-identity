#!/usr/bin/env python3
"""
Arbol ASJC completo de Scopus, reconstruido de las capturas del DOM de Scopus
(atributo data-allsubjectareas). 27 areas principales + ~330 subareas.
Estructura: cada area (2 digitos) -> subareas (4 digitos).
El codigo de 4 digitos empieza con los 2 del area padre: 31xx = Physics, etc.
"""

# {codigo_area: (nombre_area, [(codigo_sub, nombre_sub), ...])}
ASJC = {
    "10": ("Multidisciplinary", [("1000", "Multidisciplinary")]),
    "11": (
        "Agricultural and Biological Sciences",
        [
            ("1101", "Agricultural and Biological Sciences (misc)"),
    ...
    "14": (
        ...
        [
            ("1401", "Business, Management and Accounting (misc)"),
            ("1402", "Accounting"),
    ...
    "19": (
        ...
        [
            ("1901", "Earth and Planetary Sciences (misc)"),
    ...
    "20": (
        ...
        [
            ("2001", "Economics, Econometrics and Finance (misc)"),
    ...
    "23": (
        ...
        [
            ("2301", "Environmental Science (misc)"),
    ...
    "24": (
        ...
        [
            ("2401", "Immunology and Microbiology (misc)"),
    ...
    "30": (
        ...
        [
            ("3001", "Pharmacology, Toxicology and Pharmaceutics (misc)"),
            ("3002", "Drug Discovery"),
    ...
    "31": (
        ...
        [
            ("3101", "Physics and Astronomy (misc)"),
...
# Los 4 clusters de nivel superior (de tu investigacion)
CLUSTERS = {
    "Life Sciences": ["11", "13", "24", "28", "34"],
    "Physical Sciences": [
        "15",
        "16",
        "17",
    ...
    "Health Sciences": ["27", "29", "30", "35"],
    "Social Sciences & Humanities": ["12", "14", "20", "32", "33"],
    "Multidisciplinary": ["10"],
}


def subarea_lookup(code: str) -> tuple[str, str] | None:
    """Devuelve (nombre_area, nombre_sub) para un código ASJC de 4 dígitos."""
    area = code[:2]
    if area not in ASJC:
        return None
    area_name, subs = ASJC[area]
    for c, name in subs:
        if c == code:
...
if __name__ == "__main__":
    n_areas = len(ASJC)
    n_sub = sum(len(v[1]) for v in ASJC.values())
    print(f"ASJC reconstruido: {n_areas} areas principales, {n_sub} subareas")
    # Sanity: los subfields que validamos
    for code in ["3109", "1202", "3312", "2743", "3106"]:
        area = code[:2]
        found = next((n for c, n in ASJC[area][1] if c == code), "?")
        print(f"  {code} = {found}  (area {area}: {ASJC[area][0]})")
